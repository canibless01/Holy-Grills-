...
def sales_analytics():
    ...
    orders = (
        db.table("orders")
        .select("id,total,subtotal,status,payment_method,created_at")
...
def hp_analytics():
    ...
    earned = sum(t["amount"] for t in hp_txns if t.get("type") == "earn" and t["amount"] > 0 and t.get("status") == "active")
    ...
    overflow = sum(t["amount"] for t in hp_txns if t.get("status") == "overflow" and t["amount"] > 0)

    tiers = db.table("tiers").select("id,name").order("sort_order").execute()
    tier_distribution = []
    for tier in tiers:
        count = db.table("user_tiers").select("id").eq("tier_id", tier["id"]).eq("is_current", "true").execute()
...
def referral_analytics():
    ...
    """
    db = get_db()
    all_referrals = db.table("referrals").select("id,hp_awarded,hp_awarded_at").execute()
...
def dashboard_summary():
    ...
    orders_today = (
        db.table("orders")
        .select("id,status,total,payment_method,delivery_window_id,batch_id,created_at")
    ...
    revenue_today   = sum(float(o.get("total", 0)) for o in delivered_today)
...
def orders_analytics():
    ...
    orders = (
        db.table("orders")
        .select("id,status,delivery_window_id,batch_id,created_at,total")
        .gte("created_at", from_date)
        .lte("created_at", to_date + "T23:59:59Z")
        .execute()
    )
...
def marketplace_analytics():
    ...
    """
    db = get_db()
    purchases = db.table("marketplace_purchases").select("id,amount_paid_naira,hp_spent,used_hp_pricing").execute()
    total_revenue = sum(float(p.get("amount_paid_naira", 0)) for p in purchases)
    hp_discount_count = len([p for p in purchases if p.get("used_hp_pricing")])

    listings = db.table("marketplace_listings").select("id,title,is_out_of_stock,listing_type").eq("is_active", "true").execute()
    low_stock = []
    for l in listings:
        if l.get("listing_type") == "code":
            codes = db.table("marketplace_codes").select("id").eq("listing_id", l["id"]).eq("status", "available").execute()
            if len(codes) <= 5:
