...
units_bp = Blueprint("measurement_units", __name__)
stock_bp = Blueprint("stock_items", __name__)


def _alert_admin_low_stock(stock_item: dict, new_balance: float):
    """Trigger low stock alert notification to all admin profiles for the item's campus."""
    try:
        from app.services.notification_service import send_notification
        db = get_user_client()
        campus_id = stock_item.get("campus_id")
        admins_q = db.table("profiles").select("id,campus_id").in_("role", list(ADMIN_ROLES)).eq("is_active", True)
        if campus_id:
            admins_q = admins_q.eq("campus_id", campus_id)
        admins = admins_q.execute() or []

        item_name = stock_item.get("name", "Stock Item")
        threshold = stock_item.get("low_stock_threshold", 0)

        for admin in admins:
            send_notification(
                user_id=admin["id"],
                notif_type="low_stock_ingredient",
                title="Low Stock Alert",
                body=f"Low stock alert: '{item_name}' balance ({new_balance}) is below threshold ({threshold}).",
                template_data={"name": item_name, "balance": new_balance, "threshold": threshold},
                reference_id=stock_item.get("id"),
                reference_type="stock_item",
                campus_id=campus_id,
            )
    except Exception as e:
        logger.warning("_alert_admin_low_stock failed: %s", e)
...
def list_measurement_units():
    """
    Get all measurement units (spoon, sachet, bag, etc.).
    ---
    tags: [Stock]
    responses:
      200:
    ...
    """
    db = get_user_client()
    rows = db.table("measurement_units").select("id,name").order("name").execute() or []
...
def create_stock_item():
    """
    Create a new stock item / ingredient. Admin only.
    ---
    tags: [Stock]
    parameters:
      - in: body
        ...
        required: true
        schema:
          required: [name, purchase_unit_id, usage_unit_id, conversion_factor, low_stock_threshold]
          properties:
            name: {type: string}
            purchase_unit_id: {type: string}
            usage_unit_id: {type: string}
    ...
    if campus_id:
        item_record["campus_id"] = campus_id

    result = db.table("stock_items").insert(item_record).execute()
...
def list_stock_items():
    """
    Get all stock items with live balance and low stock indicator.
    ---
    tags: [Stock]
    responses:
      200:
    ...
    for item in items:
        ...
        thresh = float(item.get("low_stock_threshold") or 0)
        item["is_low_stock"] = bal < thresh

    return jsonify(items), 200


@stock_bp.route("/<stock_item_id>/purchase", methods=["POST"])
@require_role("admin")
def log_stock_purchase(stock_item_id):
    """
    Log new stock arriving. Quantity is entered in purchase_unit.
    ---
    tags: [Stock]
    parameters:
      - in: path
    ...
    item_rows = db.table("stock_items").select("*").eq("id", stock_item_id).execute() or []
    if not item_rows:
        return jsonify({"error": "Stock item not found"}), 404
    item = item_rows[0]

    qty_entered = float(data["quantity"])
    cost = float(data.get("cost", 0)) if data.get("cost") is not None else None
    conv_factor = float(item.get("conversion_factor") or 1.0)
    usage_qty = qty_entered * conv_factor

    curr_bal = float(item.get("current_balance") or 0)
    new_bal = curr_bal + usage_qty

    db.table("stock_items").eq("id", stock_item_id).update({
    ...
    ledger_entry = {
        ...
        "type": "purchase",
        "quantity_entered": qty_entered,
        "unit_id": item.get("purchase_unit_id"),
        "quantity_in_usage_units": usage_qty,
        "cost": cost,
        "logged_by": g.user_id,
        "notes": data.get("notes"),
    }
    db.table("stock_ledger").insert(ledger_entry).execute()

    return jsonify({
        "message": "Stock purchase logged successfully",
        "stock_item_id": stock_item_id,
        "current_balance": new_bal,
        "added_usage_units": usage_qty,
    }), 200


@stock_bp.route("/<stock_item_id>/usage", methods=["POST"])
@require_role("admin")
def log_stock_usage(stock_item_id):
    """
    Log stock usage, waste, or correction. Quantity is in usage_unit.
    ---
    tags: [Stock]
    parameters:
      - in: path
      ...
      - in: body
        ...
        schema:
          ...
          properties:
            quantity: {type: number}
            type: {type: string, enum: [usage, waste, correction], default: usage}
            notes: {type: string}
    responses:
      200:
        description: Usage logged and low stock alert evaluated
    """
    db = get_user_client()
    data = request.get_json(force=True) or {}
    if data.get("quantity") is None:
    ...
    entry_type = data.get("type", "usage")
    if entry_type not in ("usage", "waste", "correction"):
        return jsonify({"error": "type must be one of: usage, waste, correction"}), 400
    ...
    ledger_entry = {
        ...
        "type": entry_type,
        "quantity_entered": qty_entered,
        "unit_id": item.get("usage_unit_id"),
        "quantity_in_usage_units": qty_entered,
        "cost": None,
        "logged_by": g.user_id,
        "notes": data.get("notes"),
    }
    ...
    thresh = float(item.get("low_stock_threshold") or 0)
    is_low_stock = new_bal < thresh
    if is_low_stock:
        _alert_admin_low_stock(item, new_bal)

    return jsonify({
        "message": "Stock usage logged successfully",
        "stock_item_id": stock_item_id,
        "current_balance": new_bal,
...
@stock_bp.route("/<stock_item_id>/ledger", methods=["GET"])
@require_role("admin")
def get_stock_ledger(stock_item_id):
    """
    Get full ledger history for a stock item.
    ---
    tags: [Stock]
    parameters:
      - in: path
    ...
    responses:
      200:
        description: Stock item ledger history
    """
    db = get_user_client()
    item_rows = db.table("stock_items").select("id").eq("id", stock_item_id).execute() or []
    if not item_rows:
        return jsonify({"error": "Stock item not found"}), 404

    ledger_rows = (
        db.table("stock_ledger")
        .select("*,profiles!logged_by(full_name,email),measurement_units!unit_id(name)")
        .eq("stock_item_id", stock_item_id)
        .order("created_at", ascending=False)
        .execute()
    ) or []
    return jsonify({"ledger": ledger_rows, "count": len(ledger_rows)}), 200
