...
def complete_challenge(challenge_id):
    ...
    max_reward = current_app.config["CHALLENGE_MAX_HP_REWARD"]
    hp_reward = min(challenge.get("hp_reward", 0), max_reward)
...
def create_challenge():
    ...
    from flask import current_app
    challenge_max = current_app.config["CHALLENGE_MAX_HP_REWARD"]
    if int(data["hp_reward"]) > challenge_max:
        return jsonify({"error": f"HP reward cannot exceed {challenge_max} HP per challenge"}), 400
    data["created_by"] = g.user_id
    data["is_active"] = data.get("is_active", False)
    CHALLENGE_COLUMNS = {
        "title", "description", "hp_reward", "starts_at", "ends_at", "type",
        "max_completions_per_user", "target_count", "criteria", "is_active", "created_by",
    }
    safe = {k: v for k, v in data.items() if k in CHALLENGE_COLUMNS}
    if "title" in safe and "name" not in safe:
        safe["name"] = safe["title"]
    if "criteria" not in safe or safe["criteria"] is None:
        safe["criteria"] = {}
    try:
        result = db.table("challenges").insert(safe)
    except Exception as exc:
        return jsonify({"error": f"Failed to create challenge: {str(exc)[:120]}"}), 400
...
def update_challenge(challenge_id):
    ...
    db = get_db()
    data = request.get_json(force=True)
    CHALLENGE_UPDATE_COLS = {"is_active", "hp_reward", "ends_at", "criteria", "description"}
    safe = {k: v for k, v in data.items() if k in CHALLENGE_UPDATE_COLS}
    result = db.table("challenges").eq("id", challenge_id).update(safe)
