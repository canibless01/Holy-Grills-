...
def paystack_webhook():
    ...
    db.table("webhook_events").insert(webhook_record)
    ...
    try:
        ...
        db.table("webhook_events").eq("idempotency_key", idempotency_key).update({
...
def _handle_dva_assign(data: dict):
    ...
    db = get_db()
    auth_users = db.table("profiles").select("id").execute()
    db.table("wallets").eq("user_id", customer.get("id")).update({
        "virtual_account_number": account.get("account_number"),
        "virtual_account_bank": account.get("bank", {}).get("name"),
        "virtual_account_name": account.get("account_name"),
        "virtual_account_ref": account.get("id"),
    })
...
def _handle_transfer(data: dict):
    ...
    wallet = db.table("wallets").select("*").eq("virtual_account_number", account_number).single().execute()
    if not wallet:
        return
