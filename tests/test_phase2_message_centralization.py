...
def test_search_users_endpoint(client):
    mock_db = MagicMock()
    mock_db.auth_get_user.return_value = {"id": "me-123", "role": "student"}
    mock_db.table.return_value.select.return_value.eq.return_value.single.return_value.execute.return_value = {
        "id": "me-123", "campus_id": "campus-a", "is_active": True
    }
    with patch("app.middleware.auth.get_db", return_value=mock_db), \
         patch("app.routes.auth.get_user_client", return_value=mock_db):
        mock_db.table.return_value.select.return_value.neq.return_value.eq.return_value.or_.return_value.limit.return_value.execute.return_value = [
            {"id": "user-456", "full_name": "Chidi", "nickname": "chidi", "email": "chidi@futa.edu.ng"}
        ]
        resp = client.get("/api/auth/users/search?q=chidi", headers={"Authorization": "Bearer fake-token"})
        assert resp.status_code == 200
        res = resp.get_json()
        assert len(res["results"]) == 1
        assert res["results"][0]["id"] == "user-456"

def test_hp_transfer_cross_campus_blocked(client):
    mock_db = MagicMock()
    mock_db.auth_get_user.return_value = {"id": "sender-1", "role": "student"}
    mock_db.table.return_value.select.return_value.eq.return_value.single.return_value.execute.side_effect = [
        {"id": "sender-1", "campus_id": "campus-a", "is_active": True},
        {"id": "recip-1", "full_name": "Recip", "campus_id": "campus-b"}
    ]
    mock_db.table.return_value.select.return_value.eq.return_value.eq.return_value.execute.return_value = [{"id": "o1"}, {"id": "o2"}, {"id": "o3"}]
    with patch("app.middleware.auth.get_db", return_value=mock_db), \
         patch("app.routes.hp.get_user_client", return_value=mock_db):
        resp = client.post(
            "/api/hp/transfer",
            headers={"Authorization": "Bearer fake-token"},
            json={"recipient_id": "recip-1", "amount": 20}
        )
        assert resp.status_code == 400
        assert resp.get_json()["error"] == "You can only transfer HP to students on your own campus."

def test_multiplier_broadcast_campus_scoped():
    mock_db = MagicMock()
    res_mock = MagicMock()
    res_mock.__iter__.return_value = iter([{"id": "u1"}])
    mock_db.table.return_value.select.return_value.eq.return_value.eq.return_value.execute.return_value = res_mock

    from app.routes.admin_gifts import _broadcast_multiplier_event
    with patch("app.services.notification_service.send_notification") as mock_notif:
        _broadcast_multiplier_event(mock_db, 2.0, campus_id="campus-xyz")
